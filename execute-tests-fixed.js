    ...
    try {
        ...
        for (const xssInput of xssTests) {
            ...
            if (readError) {
                ...
                // Verificar sanitización - debería remover todas las etiquetas HTML
                const hasDangerousTags = /<[^>]*>/i.test(savedData.nombre_archivo);
            ...
            // Limpiar (sin .catch que causa error)
            try {
                await supabase
                    .from('certificados_tributarios')
                    .delete()
                ...
                console.log('⚠️ Error limpiando test:', cleanupError.message);
            }
        }
